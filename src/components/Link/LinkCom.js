...
export default function LinkCom() {
        ...
        .map((item, index) => (
          <div key={index} className="py-6 pl-6 pr-16 bg-white shadow-sm relative cursor-pointer rounded-sm mb-4">
            <div className="py-2 flex justify-between flex-col">
              <div className="skeleton w-1/2 mb-1" style={{ height: '15px' }}></div>
              <div className="skeleton w-1/2 " style={{ height: '15px' }}></div>
            </div>
        ...
        <Draggable key={item.id} draggableId={item.id.toString()} index={index} >
          {(provided) => (
            <div className='mb-4 relative ' key={index} {...provided.draggableProps} ref={provided.innerRef} onClick={() => {
              ...
              <div className={`py-6 pl-6 pr-16 bg-white shadow-sm relative cursor-pointer rounded-sm ${item.isHide ? 'opacity-40' : ''}`}>
                {item.isHeader
                  ?
  ...
  const renderSocialLink = () => {
    return socialList.filter(item => item.status).map((item, index) => {
