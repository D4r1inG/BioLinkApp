...
export default function DesignCom() {
  ...
  const { userProfile, themes, isCreating } = useSelector(state => state.ProfileReducer)
          ...
          <div className="flex">
            <div className="w-full flex justify-between flex-col">
              <div className="input-main-wrap overflow-hidden	rounded-sm">
                <input value={valueInput.name} type="text" name="name" placeholder="Name" className="bl-input w-full p-4 text-sm font-normal font-inter placeholder-grey hover:bg-bl-bg-grey focus:bg-white" onChange={handleChange} />
              </div>
              <div className="input-main-wrap overflow-hidden	rounded-sm">
                <input value={valueInput.bio} type="text" maxLength="80" name="bio" placeholder="Bio" className="bl-input w-full p-4 text-sm font-normal font-inter placeholder-grey hover:bg-bl-bg-grey focus:bg-white" onChange={handleChange} />
              </div>
            </div>
            <div className="flex items-center ml-8">
              <div style={{ width: '115px', height: '115px' }} className="profile-img-up relative">
                <div className=" flex-shrink-0" style={{ position: 'relative', paddingBottom: '0px', transitionDuration: '0.5s' }}>
                  <span style={{ height: '100%', width: '100%', position: 'absolute', inset: '0px' }}>
                    <canvas width="128" height="128" style={{ height: '100%', width: '100%', position: 'absolute', inset: '0px', display: 'none' }}></canvas>
                    <div className='flex-shrink-0 relative'>
                      <label style={{ width: '115px', height: '115px' }} htmlFor='desginImg' className={`${selectedImage === null ? 'block' : 'hidden'} relative flex justify-center items-center rounded-sm cursor-pointer xs:mx-auto img-upload-br`}>
                        <div className="text-gray-400 font-inter text-xs text-center mt-2">
                          Insert
                          <p>Picture</p>
                        </div>
                      </label>
                      <input className='hidden' id='desginImg' type='file' name='myImage' onChange={(e) => {
                      ...
                      <img style={{ width: '115px', height: '115px' }} className={`${selectedImage === null ? 'hidden' : 'block'}`} alt={name} src={typeof (selectedImage) !== 'string' ? window.URL.createObjectURL(new Blob([selectedImage], { type: "application/zip" })) : image} />
                    </div>
                  </span>
                </div>
              </div>
