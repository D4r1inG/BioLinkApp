...
export default function Tour({ steps, onRequestClose }) {
  ...
  useEffect(() => {
    ...
    if (tempEle === undefined || tempEle?.length === 0) {
      ...
      let modalOffSet =
        (tempElePos.bottom - tempElePos.top) / 2 - modalEle.height / 2;
      setPosition({
        top:
          window.innerHeight - tempElePos.height < modalEle.height
            ? "0"
            : tempElePos.top,
        left:
          tempElePos.left < modalEle.width
            ? tempElePos.right + 80
            : tempElePos.left - modalEle.width - 50,
  ...
  const renderDot = () => {
    return steps.map((_, itemIndex) => (
      <span
        key={itemIndex}
        className={`tour_dot mr-1 rounded-full ${
          itemIndex === index ? "bg-gray-400" : "bg-gray-200"
        }`}
      ></span>
    ));
  };
  ...
  return (
    <div className="tour" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div
        className="tour_overlay transition-all duration-500"
        style={styleTarget}
      ></div>
      <div
        className={`modal_tour shadow-md transition-all duration-700 ${
          index === 0 ? "text-center index0" : ""
        }`}
        style={position}
        ref={myModal}
      >
        <span
          className={`step_index bg-green-300 flex justify-center items-center rounded-full transition-all ${
        ...
        <svg
          ...
          className={`tour_close transition-all ${
        ...
        <p
          className={`text-center mt-4 text-2xl ${
            steps[index]?.quote ? "block" : "hidden "
          }`}
        >
          "<small className="italic text-lg">{steps[index]?.quote}</small>" @
          <small className="text-xl underline">KhanhNoi</small>
        </p>
        <div className="mt-4 flex justify-center items-center">
          <button
            className={`cursor-pointer py-1 px-3 rounded-md mr-6 ${
              index === 0 ? "bg-gray-200 hidden" : "bg-red-100 block"
          ...
          <button
            className={`cursor-pointer py-1 px-3 rounded-md ${
              index === 0 ? "" : "ml-6"
            } bg-green-100 hover:scale-110 transition-all`}
