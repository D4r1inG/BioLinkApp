...
export default function FirstTimeLogin() {
  ...
  const handleSubmit = () => {
    if (nameRef.current.value !== '' && bioRef.current.value !== '') {
      dispatch(createProfile(nameRef.current.value, bioRef.current.value))
    }else{
      openNotification('error', 'Oppsy! Something went wrong!', 'Please fill all the field')
    }
  }
      ...
      <div className='mt-16  h-full' style={{ width: '400px' }}>
        <div className='bg-white p-8 rounded-md'>
          <div className="flex w-full">
            <div className=" font-bold text-xl w-full">Welcome to GHTKlink</div>
          </div>
          <div className='mt-6'>
            <div className="input-main-wrap overflow-hidden	rounded-sm">
              <input ref={nameRef} type="text" placeholder="Name" className="bl-input w-full p-4 text-sm font-normal font-inter placeholder-grey hover:bg-bl-bg-grey focus:bg-white" />
            </div>
          </div>

          <div className="relative bl-input-with-suffix-wrap mt-4">
            <div className="input-main-wrap overflow-hidden	rounded-sm">
              <input ref={bioRef} placeholder="Bio" type='text' className="bl-input w-full p-4 text-sm font-normal font-inter placeholder-grey hover:bg-bl-bg-grey focus:bg-white" />
            </div>
            ...
            <button className="button-primary font-semibold font-inter  text-white rounded-sm leading-4 relative flex justify-center items-center mt-6 w-full uppercase tracking-wider" style={{ height: '40px' }} onClick={handleSubmit}>
              <span className={`${!loading ? 'block' : 'hidden'}`}>Continue</span>
