...
export default function Profile(props) {
  const { userProfile, themes } = useSelector((state) => state.ProfileReducer);
  const { linkList, socialList } = useSelector((state) => state.LinkReducer);
  const { name, bio, image, activeDesign, showLogo, showNSFW } = userProfile;
  const dispatch = useDispatch();

  const [idVisible, setIdVisible] = useState([]);
  const theme = themes.find((item) => item.id === activeDesign);
  const [warning, setWarning] = useState(showNSFW);
  ...
  const renderImg = (item) => {
    switch (item.pluginName) {
      ...
      default: {
        return (
          <img
            className="ml-3 rounded-full absolute left-0"
            src={item.image}
            alt={item.name}
            style={{ width: "40px", height: "40px", top: "10px" }}
          />
        );
      }
    }
  ...
  const renderSocialList = (list) => {
    return list.map((item, index) => {
      return (
        <div
          key={index}
          className="page-social relative mx-3 mb-3"
        ...
        if (item.isHeader) {
          return (
            <div
              ...
              className="text-center font-bold text-base mt-9 limit-one-line break-all overflow-hidden"
            >
              {item.title}
            </div>
          );
          ...
          return (
            <div
              key={index}
              className="my-4 relative transition-all hover:scale-105"
              onClick={() => {
            ...
            >
              <div
                style={pageItem}
                className="flex justify-center items-center pill-item"
              ></div>
              <div
                style={{ minHeight: "60px" }}
                className="z-10 py-2 cursor-pointer flex justify-between items-center relative"
              >
                {renderImg(item)}
                {/* <img className='ml-3 rounded-full' src={item.imgSrc} alt="youtube" style={{ width: '40px', height: '40px' }} /> */}
                <span
                  className="item-title text-center limit-one-line break-all pl-3 pr-12 "
                  style={{
                ...
                <svg
                  style={{
                    transform:
                      idVisible.indexOf(item.id) !== -1
                        ? "rotate(0deg)"
                        : "rotate(-90deg)",
                    position: "relative",
                    marginBottom: 0,
                  }}
                  className="embed-ind-arrow-icon embed-ind-arrow"
                  fill={theme?.colorLink}
                  viewBox="0 0 16 16"
              ...
              <div
                className={`embed-wrap relative ${
                  idVisible.indexOf(item.id) !== -1 ? "py-4" : "py-0"
              ...
              >
                <MediaEmbed
                  ...
                  hide={idVisible.indexOf(item.id) !== -1 ? false : true}
                />
              </div>
            </div>
          );
          ...
          return (
            ...
            >
              ...
              <a
                ...
                href={item.url}
                target="_blank"
                className="z-10 py-3 cursor-pointer flex justify-center items-center relative"
                onClick={() => {
              ...
              >
                {renderImg(item)}
                <span
                  className="item-title text-center limit-one-line break-all overflow-hidden px-4"
                  style={{
                    color: theme?.colorLink,
                    fontFamily: theme?.fontFamily,
                  }}
  ...
  return (
    <div id="profile">
      <div
        className={`${
          warning ? "flex" : "hidden"
        } page-overlay justify-center items-center flex-col text-center`}
      >
        <svg
          width="81"
          height="70"
        ...
        >
          <path
            d="M31.445 44.048a12.672 12.672 0 01-3.753-9.043A12.818 12.818 0 0140.53 22.159c3.517 0 6.755 1.424 9.047 3.753M53.13 37.281a12.812 12.812 0 01-10.294 10.314"
            stroke={theme?.colorHeader}
            strokeWidth="5"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            d="M18.844 56.647C12.404 51.592 6.952 44.207 3 35.002c3.992-9.247 9.482-16.672 15.962-21.768C25.4 8.138 32.829 5.371 40.53 5.371c7.745 0 15.17 2.808 21.65 7.94M70.751 22.236c2.792 3.709 5.246 7.99 7.311 12.765-7.98 18.489-22.083 29.626-37.53 29.626a32.38 32.38 0 01-10.273-1.676M72.531 3l-64 64"
            stroke={theme?.colorHeader}
            strokeWidth="5"
            strokeLinecap="round"
            strokeLinejoin="round"
        ...
        <h1
          style={{ color: theme?.colorHeader }}
          className="page-text-color mt-8 text-center page-overlay-title"
        >
          Sensitive Content
        </h1>
        <div
          style={{ color: theme?.colorHeader }}
          className="text-lg page-text-color mt-2 text-center page-overlay-text"
        >
          You must be of a legal age to view this content
        </div>
        <button
          style={{ color: theme?.colorHeader, borderColor: theme?.colorHeader }}
          className="mt-8 page-text-color page-overlay-btn"
          onClick={() => {
            setWarning(false);
          }}
        >
      ...
      <div className=" w-full min-h-screen flex justify-center relative">
        ...
        <img
          className={`pride-page-image ${
            !theme?.backgroundImg ? "hidden" : "block"
          }`}
          style={{ zIndex: -1 }}
        ...
        <div style={{ width: "670px" }} className="mt-12 pb-32">
          <img
            ...
            className="display-image m-auto rounded-full"
            src={image}
            alt="D4rl1nG"
          />
          <h2
          ...
          <div
            className="text-base font-normal mt-3 text-center"
            style={{ color: theme?.colorHeader, fontFamily: theme?.fontFamily }}
          >
            {bio}
          </div>
          ...
          <div className="mt-6 px-4">{renderLink(linkList)}</div>

          <div
            className={`page-logo text-center ${showLogo ? "block" : "hidden"}`}
          >
            <a href="#">
              <svg
                width="32"
      ...
      <div
        className="chat_box rounded-full"
        onClick={() => {
          setIsOpen(!isOpen);
        }}
      >
